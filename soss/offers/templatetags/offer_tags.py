...
def choice_buttons(choice):
    return [
    ('<button type="submit" value="1" name="choose_self" class="chooser"><span>I lose <strong>%s</strong></span></button>' % to_currency(choice.self_offer)),
    ('<button type="submit" value="1" name="choose_other" class="chooser"><span>Other person loses <strong>%s</strong></span></button>' % to_currency(choice.other_offer))
    ]
...
def to_currency(cents):
    return "%d.%02d shocks" % (int(cents/100), int(cents%100))
