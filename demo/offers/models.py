...
class Subject(StampedTrackedModel):
    ...
    def plot_logistic(self):
        ...
        choice_pct = numpy.mean(choices_np)
        if choice_pct > .9:
            ip = 3
        if choice_pct < .1:
            ip = -3
        ...
        val_other = 1.0/numpy.exp(ip)
        ip_label = "Estimated Indifference Point: $%.2f" % val_other
        
        if ip == 3:
            betas = [100.0, 0.0]
            std_err = [0.0, 0.0]
            ip_label = "Capped Indifference Point: <$0.05"
        if ip == -3:
            betas = [-100.0, 0.0]
            std_err = [0.0, 0.0]
            ip_label = "Capped Indifference Point: >$20"
        
        x = numpy.arange(-3,3,.005)
        bx = betas[1]*x
        y = 1/(1+numpy.exp(betas[0]+bx))
        ...
        #sorted_lr_np = numpy.hstack(([numpy.min(sort_lr_np)]*10, sort_lr_np, [numpy.max(sort_lr_np)]*10))
        #sorted_choices_for_other = numpy.hstack(([numpy.min(sorted_choices_for_other)]*10, sorted_choices_for_other, [numpy.max(sorted_choices_for_other)]*10))
        
        bin_lr_np = numpy.convolve(smooth_window,sorted_lr_np,mode='same')[10:-10]
        bin_choices_for_other= numpy.convolve(smooth_window,sorted_choices_for_other,mode='same')[10:-10]

        plt.plot(sorted_lr_np,sorted_choices_for_other,'b|',markeredgecolor=[.65, .65, .9],markeredgewidth=1.5, markersize=25)
        plt.fill_between(x,y_ci_max,y_ci_min,where=None,facecolor=[1,.85,.85],edgecolor=[1,.65,.65], alpha=.85)

        plt.plot(x,y,'r')
        ip_thinger = plt.plot(ip, 0.5, 'ro', markersize=10, markeredgewidth=0, label=ip_label)
        plt.legend(loc='upper center', bbox_to_anchor=(0, 0, 1, 1.1), numpoints=1)

        hTitle  = plt.title ('');
        hXLabel = plt.xlabel('Ratio of SelfGets:OtherGets (log-scaled)');
        hYLabel = plt.ylabel('Percentage of Choices for a Stranger to Receive Money');
        
        hXTics  = plt.xticks([-3,-2,-1,0,1,2,3],['20:1','7.4:1','2.7:1','1:1','1:2.7','1:7.4','1:20'] )
        hYTics  = plt.yticks(
            numpy.linspace(0, 1, 11), 
            ["%s%%" % int(n) for n in numpy.linspace(0, 100, 11)])
