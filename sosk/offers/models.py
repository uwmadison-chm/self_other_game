...
class Session(StampedTrackedModel):
    ...
    total_trials = models.IntegerField(default=50)
    ...
    maximum_offer = models.IntegerField(default=100)
...
class Subject(StampedTrackedModel):
    ...
    def diagnostic_links(self):
        return mark_safe(
            "<a href=\"%s\">Logistic</a> &middot; <a href=\"%s\">Scatter</a>" % (
            reverse(
                'offers.views.plot_logistic_png', 
                args=[self.session_and_subject]),
            reverse(
                'offers.views.plot_scatter_png', 
                args=[self.session_and_subject, 
                'logratio_trial_choseSelf'])))
    diagnostic_links.allow_tags = True

    def session_diagnostic_link(self):
        return mark_safe("<a href=\"%s\">Session Logistic</a>" % (
            reverse('offers.views.plot_logistic_multiple_png', args=[self.session.session_number_ext])))
    ...
    def plot_logistic(self):
        ...
        ip = numpy.exp(-betas[0]/betas[1])
        # This does not work -- what's val_other?
        ...
        hXLabel = plt.xlabel('Ratio of SelfGets:OtherGets (axis is ln-scaled)');
